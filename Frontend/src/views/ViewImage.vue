<template>
  <div>
    <div class="card" style="background-color: #9fa9a3;">
      <div
        class="card-header d-flex justify-content-between align-items-center"
      >
        <h2 class="mx-auto">{{ post.title }}</h2>
        <b-button variant="danger"  @click="deletePost" v-if="userDB != '' && post.userId === userDB.data._id">
          Delete
          <BIconTrash2 />
        </b-button>
      </div>
      ...
      >
        <b-button variant="success" @click="postLike()" >
          Like
          <BIconHandThumbsUp />
        </b-button>
        <p>
    ...
    <div class="card mt-2" style="background-color: #9fa9a3;">
      ...
      <b-navbar toggleable >
        <h1>Comments <BIconChatSquareDotsFill/></h1> 
        <b-navbar-toggle target="navbar-toggle-collapse" style="background-color: #878f99;">
          ...
          <b-navbar-nav class="ml-auto">
            <form method="POST" @submit.prevent="sendComment(comment)">
              <div class="form-group">
                <textarea class="form-control" name="10" rows="2" placeholder="Write a comment" v-model="comment.comment"></textarea>
              </div>
              <div class="form-group">
        ...
        <ul class="list-group p-4" v-for="(comment, index) in comments" :key="index">
            <li class="list-group-item">
              <div class="row">
                ...
                <b-avatar v-if="comment.avatar != ''" size="4rem" :src="comment.avatar" class="mx-auto"></b-avatar>
...
export default {
  ...
  methods: {
    deletePost(){
      ...
      if(res){
        this.axios.delete(`/images/${this.image_id}`)
        .then((result) => {
          console.log(result);
    ...
    postLike(){
      ...
      this.axios.post(`/images/${this.image_id}/like`, config)
        .then((result) => {
          console.log(result);
    ...
    sendComment(){
      ...
      this.axios.post(`/images/${this.image_id}/comment`, this.comment, config)
        .then(res => {
